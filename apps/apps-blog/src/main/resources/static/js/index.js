...
function parseBlog(blogs, page, pagesize) {
    ...
    $.each(blogs, function (index, blog) {
        ...
        var $a1 = $("<a></a>");
        $a1.prop("href", "/blog/blogs/"+blog.author+"/article/" + blog.id);
        var $img = $("<img>");
        $img.prop("src", blog.uri);
        var $h3 = $("<h3 class='blogtitle'></h3>");
        var $a2 = $("<a></a>");
        $a2.prop("href", "/blog/blogs/"+blog.author+"/article/" + blog.id);
        $a2.text(blog.title);
        ...
        var $a3 = $("<a></a>");
        $a3.text("阅读全文");
        $a3.prop("href", "/blog/blogs/"+blog.author+"/article/" + blog.id);
        $li.append($span);
        $span.append($a1);
        $a1.append($img);
        $li.append($h3);
